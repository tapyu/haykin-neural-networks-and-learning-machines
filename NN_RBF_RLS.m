function [c,spread]= NN_RBF_RLS(dist, num_hd)
...
%%>>>>>>>>>> Parameters For RLS >>>>>>>>>>>>>>>>>>
w = zeros(num_ou,num_hd);%rand(num_ou,num_hd)./2  - 0.25; % Initial weights
sig = 0.01;
P = sig^(-1)*eye(num_hd);
lambda = 1;
num_train_min = 10;
...
fprintf(' Center coordinates: [%4.1f, %4.1f]\n', [c(1,:),c(2,:)]);
fprintf('    Spread(width)  : %4.1f\n',spread);
fprintf('  ------------------------------------\n');
...
for i = 1:num_te,
    ...
    %if mysign(o) - data_shuffled(3,i+num_tr) ~= 0,
        err = err + 1;
    end
end
fprintf('  ------------------------------------\n');
...
%     plot(sigma(i)*cos(t)+c(1,i),sigma(i)*sin(t)+c(2,i),'w');
% end
